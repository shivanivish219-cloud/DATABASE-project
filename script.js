...
document.addEventListener('DOMContentLoaded', () => {
  ...
  si.addEventListener('input', () => {
    document.getElementById('sClear').style.display = si.value ? 'flex' : 'none';
  });
...
function makeCard(p, idx) {
  ...
  const stTxt   = outSt ? '❌ Out of stock' : lowSt ? `Only ${p.stock} left` : 'In Stock';
...
function renderPagination() {
  ...
  if (totalPages <= 1) { block.style.display = 'none'; return; }
  ...
  // PREV
  const prev = mkPgBtn('← Prev', S.page === 1, () => toPage(S.page - 1), 'nav-arrow');
  ...
  smartPageNums(S.page, totalPages).forEach(n => {
    if (n === '…') {
      ...
      s.textContent = '…';
      s.style.cssText = 'padding:0 6px;color:var(--text3);font-size:0.85rem;display:flex;align-items:center;';
      links.appendChild(s);
    } else {
  ...
  // NEXT
  const next = mkPgBtn('Next →', S.page === totalPages, () => toPage(S.page + 1), 'nav-arrow');
...
function smartPageNums(cur, total) {
  ...
  if (cur >= total - 4)  return [1,2,'…',total-5,total-4,total-3,total-2,total-1,total];
...
function renderActiveChips() {
  ...
  if (S.category !== 'all')  chips.push({ label: S.category, clear: () => { S.category = 'all'; document.querySelector('[name=fcat][value=all]').checked = true; syncAndFetch(); } });
  if (S.search)              chips.push({ label: `"${S.search}"`, clear: () => { S.search = ''; document.getElementById('searchInput').value = ''; document.getElementById('sClear').style.display='none'; syncAndFetch(); } });
  if (S.minPrice > 0)        chips.push({ label: `Min ₹${fmt(S.minPrice)}`, clear: () => { S.minPrice = 0; document.getElementById('sliderMin').value = 0; syncSliders(); syncAndFetch(); } });
  if (S.maxPrice < 200000)   chips.push({ label: `Max ₹${fmt(S.maxPrice)}`, clear: () => { S.maxPrice = 200000; document.getElementById('sliderMax').value = 200000; syncSliders(); syncAndFetch(); } });
  if (S.rating)              chips.push({ label: `${S.rating}+ Stars`, clear: () => { S.rating = ''; document.querySelector('[name=frat][value=""]').checked = true; syncAndFetch(); } });
...
function navCat(cat, btn) {
  ...
  if (btn) btn.classList.add('active');
    ...
    if (found) { document.querySelectorAll('.nl').forEach(b => b.classList.remove('active')); found.classList.add('active'); }
  }
...
function clearFilters() {
  S.category = 'all'; S.search = ''; S.minPrice = 0; S.maxPrice = 200000;
  S.rating = ''; S.sortBy = 'createdAt'; S.sortOrder = 'DESC'; S.page = 1;
  ...
  document.querySelectorAll('.nl').forEach(b => b.classList.toggle('active', b.dataset.cat === 'all'));
  ...
  const sortDef = document.querySelector('[name=fsort][value="createdAt|DESC"]');
...
function openCartModal() { if (!cartOpen) toggleCart(); }
      ...
      <div class="dm-info">
        ...
        <div class="dm-stock ${lowSt ? 'low' : ''}">
          ${p.stock === 0 ? '❌ Out of Stock'
            : lowSt ? `⚠️ Hurry! Only ${p.stock} left`
...
function handleSignup() {
  ...
  if (!name || !email || !pass) { toast('Please fill all fields!', 'err'); return; }
...
function openAddProductModal() {
  ['apTitle','apBrand','apDesc','apPrice','apStock','apRating'].forEach(id => {
    const el = document.getElementById(id);
    if (el) el.value = '';
  });
...
async function submitProduct() {
  ...
  if (price < 0)        { toast('Price must be positive.', 'err'); return; }
  if (rating < 1 || rating > 5) { toast('Rating must be 1–5.', 'err'); return; }

  const payload = { title, brand, description: desc, price, category, stock, rating };
...
/* ── GO HOME ── */
function goHome() {
  S.search = ''; S.category = 'all'; S.page = 1;
  document.getElementById('searchInput').value = '';
  document.getElementById('sClear').style.display = 'none';
  document.getElementById('hero').style.display = '';
  document.getElementById('shopCats').style.display = '';
